...
def project_name(project_root: Path) -> str:
    ...
    with open(pyproject_path, "rb") as f:
        data = tomllib.load(f)
        return str(data.get("project", {}).get("name", "unknown_project"))


def test_src_layout_exists(project_root: Path, project_name: str) -> None:
    ...
    assert (package_dir / "__init__.py").exists(), (
        f"'src/{package_name}/__init__.py' is missing."
    )


def test_github_workflows_exists(project_root: Path) -> None:
