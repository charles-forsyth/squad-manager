...
try:
    ...
    import tomli as tomllib  # For Python < 3.11, though design doc specifies 3.12+
...
def test_pyproject_has_project_section(pyproject_data):
    """Test that the [project] section exists and has required fields."""
    assert "project" in pyproject_data, "pyproject.toml is missing the [project] section."
...
def test_pyproject_dev_dependencies(pyproject_data):
...
def test_pyproject_ruff_config(pyproject_data):
...
def test_pyproject_mypy_config(pyproject_data):
    ...
    assert mypy_config.get("python_version") == "3.12", "mypy python_version should be 3.12"
